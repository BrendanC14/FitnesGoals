package com.cutlerdevelopment.fitnessgoals.Constants;

public class Numbers {

    public static final int MOCKED_AVERAGE_STEPS = 6000;

    //UI Numbers
    public static final long FIRST_MENU_JAMES_ANIM_DURATION = 1000;
    //This is how far down James the speech bubble drops as a fraction. Eg 2 = 1/2. 4 = 1/4.
    public static final float FIRST_MENU_SPEECH_BUBBLE_BOUNCE_POSITION = 3;
    public static final long FIRST_MENU_SPEECH_BUBBLE_ANIM_DURATION = 1000;
    public static final long FIRST_MENU_SPEECH_BUBBLE_BOUNCE_ANIM_DURATION = 500;
    public static final long FIRST_MENU_ANIM_DURATION = 1000;
    public static final long FIRST_MENU_MARGIN_PADDING = 32;
    public static final float SPEECH_BUBBLE_PADDING_OFFSET = 114;
    public static final int TM_MAIN_MENU_SMALL_TABLE_ROW_NUMBER = 5;
    public static final int TM_MAIN_MENU_SMALL_FIXTURES_TABLE_ROW_NUMBER = 5;
    public static final int TM_MAIN_MENU_BIG_FIXTURES_TABLE_ROW_NUMBER = 15;
    public static final int TM_MAIN_MENU_BIG_TABLE_ROW_NUMBER = 15;
    public static final long TM_MAIN_MENU_EXPAND_TABLE_ANIM_DURATION = 1000;
    public static final int TM_MAIN_MENU_STEP_PROGRESS_ANIM_DURATION = 1000;
    public static final long TM_MAIN_MENU_GOLDEN_FOOTBALL_ANIM_DURATION = 1000;
    public static final long TM_MAIN_MENU_CARD_INTRO_DURATION = 2000;
    public static final long MATCH_SETUP_TACTIC_MOVE_SPEED = 1000;

    //Team
    public static final int TOP_TEAM_STEPS = 20000;
    public static final int TEAM_STEP_REDUCTION_PER_POSITION = 200;
    public static final int DIFFERENCE_BETWEEN_TEAM_MIN_MAX = 3000;

    //MatchEngine
    public static final int TM_GOAL = 500;
    public static final int MATCH_BUFFER = 1000;

    //Settings
    public static final int MAX_NUM_STEPS_TARGET = 20;
    public static final int MAX_NUM_DAYS_BETWEEN = 8;
}
